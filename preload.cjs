...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  cancelDownload: (serviceType) => {
    console.log(
      `Preload: Sending docker:cancel-download IPC message for ${serviceType}.`,
    );
  ...
  onStatusChange: (callback) =>
    createListener("openfork_client:status", callback),
  onProviderId: (callback) =>
    createListener("openfork_client:provider-id", callback),
  onDockerProgress: (callback) =>
    createListener("openfork_client:docker-progress", callback),
  onJobStatus: (callback) =>
    createListener("openfork_client:job-status", callback),
  onDiskSpaceError: (callback) =>
    createListener("openfork_client:disk-space-error", callback),
  ...
  onForceRefresh: (callback) =>
    createVoidListener("auth:force-refresh", callback),

  // Force logout handling (permanent auth failure)
  onForceLogout: (callback) =>
    createVoidListener("auth:force-logout", callback),
  ...
  getDockerImageCacheUsage: () =>
    ipcRenderer.invoke("docker:get-image-cache-usage"),
  onImageEvicted: (callback) =>
    createListener("openfork_client:image-evicted", callback),
  ...
  onDockerContainersUpdate: (callback) =>
    createListener("docker:containers-update", callback),
  onDockerImagesUpdate: (callback) =>
    createListener("docker:images-update", callback),
  ...
  installEngine: (installPath) =>
    ipcRenderer.invoke("deps:install-engine", installPath),
  onInstallProgress: (callback) =>
    createListener("deps:install-progress", callback),
  ...
  fixLinuxDockerPermissions: () =>
    ipcRenderer.invoke("deps:fix-linux-docker-permissions"),
  onWslDistroMissing: (callback) =>
    createListener("docker:wsl-distro-missing", callback),
  onEngineSwitch: (callback) =>
    createListener("docker:engine-switched", callback),
  onWslRecoveryStatus: (callback) =>
    createListener("docker:wsl-recovery-status", callback),

  // Disk Management
  relocateStorage: (newDrivePath) =>
    ipcRenderer.invoke("docker:relocate-storage", newDrivePath),
  ...
  onCompactionSuggested: (callback) =>
    createVoidListener("docker:compaction-suggested", callback),
  ...
  onUpdateInstalling: (callback) =>
    createListener("update:installing", callback),
  onRequiredUpdate: (callback) => createListener("update:required", callback),
  checkUpdatePolicy: () => ipcRenderer.invoke("update:check-policy"),
  ...
  setScheduleConfig: (config) =>
    ipcRenderer.invoke("schedule:set-config", config),
  ...
  setMonetizeIdleTimeout: (minutes) =>
    ipcRenderer.invoke("monetize:set-idle-timeout", minutes),
  getMonetizeConfig: () => ipcRenderer.invoke("monetize:get-config"),
  onMonetizeCleanupEvent: (callback) =>
    createListener("monetize:cleanup-event", callback),

  // Provider custom pricing
  getProviderRate: () => ipcRenderer.invoke("monetize:get-provider-rate"),
  setProviderRate: (rate) =>
    ipcRenderer.invoke("monetize:set-provider-rate", rate),
