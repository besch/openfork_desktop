    ...
    return new Promise((resolve, reject) => {
      ...
      const req = http.request(options, (res) => {
        if (res.statusCode === 200) {
          ...
          res.on('end', () => {
            console.error(`Failed to update Python backend tokens. Status: ${res.statusCode}, Body: ${body}`);
            reject(new Error(`Failed to update tokens with status: ${res.statusCode}`));
          });
        }
      });
    ...
    try {
      ...
      const handlePythonLog = (log) => {
        if (log.startsWith("DGN_CLIENT_TOKEN_SERVER_PORT:")) {
            const portStr = log.substring("DGN_CLIENT_TOKEN_SERVER_PORT:".length).trim();
            const port = parseInt(portStr, 10);
            if (!isNaN(port)) {
              this.tokenServerPort = port;
              console.log(`Python token server is running on port ${this.tokenServerPort}`);
            }
            return;
        }
        ...
        if (log.startsWith("DGN_CLIENT_TOKENS_REFRESHED:")) {
          try {
            const jsonString = log.substring("DGN_CLIENT_TOKENS_REFRESHED:".length);
            const tokens = JSON.parse(jsonString);
            console.log("Received refreshed tokens from Python. Updating main session.");
        ...
        if (log.includes("DGN_CLIENT_AUTH_REFRESH_FAILED")) {
          console.warn("Python reported auth failure. Attempting to recover session.");
          (async () => {
            const { data: freshData, error: refreshError } = await this.supabase.auth.getSession();
            if (refreshError || !freshData.session) {
              console.error("Could not recover session. Forcing logout.", refreshError?.message);
              await this.stop();
              await this.supabase.auth.signOut();
            } else {
              console.log("Recovered session. Pushing new tokens to Python.");
              try {
                await this.updatePythonBackendTokens(freshData.session.access_token, freshData.session.refresh_token);
              } catch (updateError) {
                console.error("Failed to push updated tokens to Python. Shutting down.", updateError);
                await this.stop();
                await this.supabase.auth.signOut();
              }
            }
        ...
        this.mainWindow.webContents.send("dgn-client:log", { type: "stdout", message: log });
      };
      ...
      this.pythonProcess.stderr.on("data", (data) => {
        ...
        const logs = data.toString().split(/\r?\n/).filter(Boolean);
        logs.forEach(log => {
            this.mainWindow.webContents.send("dgn-client:log", { type: "stderr", message: log });
        });
      });
  ...
  stop() {
    return new Promise((resolve) => {
      ...
      fetch(`http://localhost:8000/shutdown`)
        .catch(error => {
          console.error(`Error sending HTTP shutdown request: ${error}. Falling back to kill.`);
